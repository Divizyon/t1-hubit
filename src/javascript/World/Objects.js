    ...
    getConvertedMesh(_children, _options = {}) {
        ...
        for (const _child of baseChildren) {
            ...
            if (_child instanceof THREE.Mesh) {
                ...
                // Eğer materyal korunacaksa parser kullanma
                if (_options.preserveMaterials) {
                    mesh = _child.clone();
                    if (_child.material) {
                        if (Array.isArray(_child.material)) {
                            mesh.material = _child.material.map(mat => mat.clone());
                        } else {
                            mesh.material = _child.material.clone();
                        }
                    }
                } else {
                    ...
                    mesh = parser.apply(_child, _options)
                }
