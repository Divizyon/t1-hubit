...
export default class World {
  ...
  start() {
    ...
    this.setKelebeklerVadisiArea(); // Kelebekler vadisi etkileşim alanını ekler
  ...
  setKelebeklerVadisi() {
    try {
      ...
      if (this.resources && 
        ...
        const modelScene = this.resources.items.kelebeklerVadisiModel.scene;
        
        if (!modelScene.children || modelScene.children.length === 0) {
          console.warn("Kelebekler Vadisi modelinin children özelliği yok veya boş, model eklenemedi.");
          return;
        }
        ...
        this.kelebeklerVadisi = this.objects.add({
          ...
          offset: new THREE.Vector3(35, -35, -0.5), // X değerini 5 birim artırdım (30->35)
          rotation: new THREE.Euler(0, 0, 0), // Düz duracak şekilde rotasyonu sıfırlıyorum
          shadow: { sizeX: 3, sizeY: 3, offsetZ: -0.6, alpha: 0.4 },
          mass: 0, // Statik bir model olduğu için kütle 0
          sleep: true, // Fizik hesaplamaları yapılmasın
          name: "Kelebekler Vadisi" // İsim parametresi eklendi
        });
  ...
  setKelebeklerVadisiArea() {
    try {
      ...
      areaLabelMesh.position.set(35, -40, 0.5); // Kelebekler Vadisi'nin önünde, Y ekseninde 5 birim aşağıda
      areaLabelMesh.matrixAutoUpdate = false;
      areaLabelMesh.updateMatrix();
      this.container.add(areaLabelMesh);
      ...
      this.kelebeklerVadisiArea = this.areas.add({
        position: new THREE.Vector2(35, -40), // Kelebekler Vadisi'nin önünde (X: 35, Y: -40)
        halfExtents: new THREE.Vector2(2, 2), // 2x2 birimlik alan
      });
      ...
      this.kelebeklerVadisiArea.on("interact", () => {
        ...
        titleEl.style.fontSize = "24px";
        titleEl.style.fontWeight = "bold";
        titleEl.textContent = "Kelebekler Vadisi Bilgi";

        // Açıklama metni
        const descriptionEl = document.createElement("p");
        descriptionEl.textContent = "Kelebekler Vadisi, Türkiye'nin en güzel doğal alanlarından biridir. Fethiye'de bulunan bu vadi, adını bölgede yaşayan çok sayıda kelebek türünden almıştır.";
        descriptionEl.style.margin = "0 0 20px 0";

        // Ek bilgi
        const additionalInfoEl = document.createElement("p");
        additionalInfoEl.textContent = "180 farklı kelebek türüne ev sahipliği yapan bu doğa harikası, dünyanın dört bir yanından turistleri kendisine çekmektedir.";
        additionalInfoEl.style.margin = "0 0 20px 0";

        // Görsel yerine link oluştur
        const linkEl = document.createElement("a");
        linkEl.href = "https://konyatropikalkelebekbahcesi.com/tr";
        linkEl.textContent = "konyatropikalkelebekbahcesi.com";
        linkEl.target = "_blank";
        linkEl.style.display = "inline-block";
        linkEl.style.padding = "12px 25px";
        linkEl.style.backgroundColor = "#3498db";
        ...
        popupBox.appendChild(titleEl);
        popupBox.appendChild(descriptionEl);
        popupBox.appendChild(additionalInfoEl);
        popupBox.appendChild(linkEl);
        popupBox.appendChild(closeButton);
        popupContainer.appendChild(popupBox);
        document.body.appendChild(popupContainer);
